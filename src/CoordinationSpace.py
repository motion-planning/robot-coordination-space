...
# Hard-coded tester for 3D space
...
def findNeighbors(point):
    ...
    for neighbor in neighbors:
        if neighbor == point or cSpace[neighbor[0]][neighbor[1]][neighbor[2]] == 1:
            neighbors.remove(neighbor)
    return neighbors

def aStarSearch():
    # Nodes are composed of: [0] costToCome, [1] costToGo, [2] coordinates in x, y, z

    closedSet = []  # Nodes already evaluated
    openSet = []  # Tentative nodes to evaluate
    heapq.heappush(openSet, [0, 0, [0, 0]])  # OpenSet initially contains start
    cameFrom = []  # Map of navigated nodes

    

    cSpace[0][0][0][4] = 0  # Cost from start along best known path

    # Estimated cost from start to goal
    fScore = costToGo(cSpace[0][0][0])

    while openSet:
        current = heapq.heappop(openSet)  # The node in openSet having the lowest f-score value
        if current[1] == [0, 0, 0]:
            print 'yes'
